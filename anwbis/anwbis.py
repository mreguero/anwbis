...
class Anwbis:
    ...
    def token(self):
        ...
        if self.project and self.args.env:
            if not self.args.from_ec2_role:
                group_name = 'corp-'+self.project+'-master-'+role
                policy_name = 'Delegated_Roles'
                role_filter = self.env+'-'+self.project+'-delegated-'+role
            else:
        ...
        if len(delegated_arn) == 0:
            if self.args.role and self.project:
                colormsg("Sorry, you are not authorized to use the role {} for project {} error".format(role, self.project))
                exit(1)
            else:
        ...
        if not self.args.nomfa:
            ...
            if self.project:
                colormsg("Assuming role {role} from project {project} using MFA device from user {session_name} ...".format(role=role, project=self.project, session_name=role_session_name), "normal")
            elif self.args.iam_delegated_role:
                colormsg("Assuming role "+ role+ " using MFA device from user "+ role_session_name+ "...", "normal")
        else:
            verbose("Assuming role "+ role_arn+ "...")
            if self.project:
                colormsg("Assuming role {role} from project {project} from user {session_name} ...".format(role=role, project=self.project, session_name=role_session_name), "normal")
            elif self.args.iam_delegated_role:
                colormsg("Assuming role {role} from user {session_name} ...".format(role=role, session_name=role_session_name), "normal")
        if self.args.get_session:
            sts_token = self.get_session_token(sts_connection, role_arn, mfa_serial_number, role_session_name, self.project, self.env, role, token_expiration, session_token_expiration)
        else:
            if os.path.isfile(os.path.expanduser('~/.anwbis')):
                ...
                with open(os.path.expanduser('~/.anwbis')) as json_file:
                    ...
                    if self.project in root_json_data and self.env in root_json_data[self.project] and role in root_json_data[self.project][env]:
                        json_data = root_json_data[self.project][self.env][role]
                        ...
                        if int(time.time()) - int(anwbis_last_timestamp) > token_expiration or self.args.refresh:
                            ...
                            sts_token = self.get_sts_token(sts_connection, role_arn, mfa_serial_number, role_session_name, self.project, self.env, role, token_expiration)
    ...
    def controller(self):
        ...
        if self.args.list:
            list_function(list_instances, self.access_key, self.session_key, self.session_token, self.region, filter_name)

        # Teleport parser for connecting to bastion

        if self.args.teleport:
            bastions = list_function('teleport', self.access_key, self.session_key, self.session_token, self.region, filter_name)
            if len(bastions) == 0:
        ...
        if os.path.isfile(os.path.expanduser(local_file_path)):
            ...
            with open(os.path.expanduser(local_file_path), 'w+') as json_file:
                ...
                root_json_data[project_name][environment_name][role_name]["anwbis_last_timestamp"] = str(int(time
                                                                                                                 .time()))
                root_json_data[project_name][environment_name][role_name]["access_key"] = self.access_key
                root_json_data[project_name][environment_name][role_name]["role_session_name"] = role_session_name
                root_json_data[project_name][environment_name][role_name]["session_key"] = self.session_key
                root_json_data[project_name][environment_name][role_name]["session_token"] = self.session_token
                root_json_data[project_name][environment_name][role_name]["region"] = self.region
    ...
    def get_sts_token(self, sts_connection, role_arn, mfa_serial_number, role_session_name, project_name, environment_name, role_name, token_expiration):
        try:
        ...
        self.save_credentials(role_session_name, project_name, environment_name, role_name)

        #and save them on the CLI config file .aws/credentials

        self.save_cli_credentials('-'.join([project_name, environment_name, role_name]))
        ...
        return { 'access_key': self.access_key, 'session_key': self.session_key, 'session_token': self.session_token, 'role_session_name': role_session_name }

    def get_session_token(self, sts_connection, role_arn, mfa_serial_number, role_session_name, project_name, environment_name, role_name, token_expiration, session_token_expiration):
        try:
        ...
        self.save_credentials(role_session_name, 'corp', 'session', credential_profile)


        #and save them on the CLI config file .aws/credentials

        self.save_cli_credentials('-'.join(['corp','session',credential_profile]))
    ...
    def login_to_fedaccount(self, role_session_name):
        ...
        json_temp_credentials += '"sessionId":"' + self.access_key + '",'
        json_temp_credentials += '"sessionKey":"' + self.session_key + '",'
        json_temp_credentials += '"sessionToken":"' + self.session_token + '"'
        json_temp_credentials += '}'
